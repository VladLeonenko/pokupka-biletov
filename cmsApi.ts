...
// Определяем существующий товар по наличию id или createdAt
const isExisting = !!(item.id || item.createdAt);

const url = `${getApiBaseUrl()}/api/products${isExisting ? `/${encodeURIComponent(item.slug)}` : ''}`;
