...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'product_categories') THEN
    ...
    -- Внешний ключ для parent_id будет добавлен позже
    -- ALTER TABLE product_categories
    --   ADD CONSTRAINT product_categories_parent_id_fkey 
    --   FOREIGN KEY (parent_id) REFERENCES product_categories(id) ON DELETE SET NULL;
  ELSE
    -- Добавляем колонки с проверкой через pg_attribute
    IF NOT EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = 'product_categories'::regclass AND attname = 'description' AND attnum > 0) THEN
      ALTER TABLE product_categories ADD COLUMN description TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = 'product_categories'::regclass AND attname = 'parent_id' AND attnum > 0) THEN
      ALTER TABLE product_categories ADD COLUMN parent_id INTEGER;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = 'product_categories'::regclass AND attname = 'sort_order' AND attnum > 0) THEN
      ALTER TABLE product_categories ADD COLUMN sort_order INTEGER DEFAULT 0;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = 'product_categories'::regclass AND attname = 'created_at' AND attnum > 0) THEN
      ALTER TABLE product_categories ADD COLUMN created_at TIMESTAMP WITHOUT TIME ZONE DEFAULT NOW();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = 'product_categories'::regclass AND attname = 'updated_at' AND attnum > 0) THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = 'product_categories'::regclass AND attname = 'is_active' AND attnum > 0) THEN
      ALTER TABLE product_categories ADD COLUMN is_active BOOLEAN DEFAULT TRUE;
    END IF;
    
    -- Внешний ключ будет добавлен позже
...
-- Индексы для product_categories (создаем отдельно)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'product_categories') THEN
    ...
    BEGIN
      IF EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = 'product_categories'::regclass AND attname = 'parent_id') THEN
    ...
    BEGIN
      IF EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = 'product_categories'::regclass AND attname = 'is_active') THEN
...
-- Внешний ключ будет добавлен позже после успешного создания всех таблиц
