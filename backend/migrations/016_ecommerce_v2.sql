...
-- Категории продуктов - создаем таблицу без is_active, потом добавим
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'product_categories') THEN
...
-- Добавляем недостающие колонки включая is_active
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'product_categories') THEN
    ...
    BEGIN
      ALTER TABLE product_categories ADD COLUMN IF NOT EXISTS parent_id INTEGER;
      EXCEPTION WHEN duplicate_column THEN NULL;
    END;
    ...
    BEGIN
      ALTER TABLE product_categories ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITHOUT TIME ZONE DEFAULT NOW();
      EXCEPTION WHEN duplicate_column THEN NULL;
    END;
    
    BEGIN
      ALTER TABLE product_categories ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITHOUT TIME ZONE DEFAULT NOW();
      EXCEPTION WHEN duplicate_column THEN NULL;
    END;
