...
// Same extraction functions as migratePagesToProducts.js
...
function extractDescriptionSection(html) {
  ...
  return { title: 'Описание', text: descMatch[1].trim() };
}
...
function extractWorkSteps(html) {
  ...
  for (const match of stepMatches) {
    steps.push({ number: match[1] || '', description: match[2]?.trim() || '' });
  }
  if (steps.length === 0) return null;
  return { title: 'Этапы работы', description: stepsMatch[1].trim(), steps };
}
...
function extractStats(html) {
  ...
  for (const match of statMatches) {
    items.push({ value: match[1]?.trim() || '', label: match[2]?.replace(/<br\s*\/?>/g, ' ').trim() || '' });
  }
  if (items.length === 0) return null;
  return { title: 'О рекламе в цифрах', description: statsMatch[1].trim(), items };
}
...
function extractTeam(html) {
  ...
  return { title: 'Команда', description: teamMatch[1].trim(), members };
}
...
function extractRelatedServices(html) {
  ...
  for (const match of serviceMatches) {
    services.push({ title: match[2]?.trim() || '', imageUrl: match[1]?.replace(/@img\//, '/legacy/img/') || '' });
  }
  if (services.length === 0) return null;
  return { title: 'Другие услуги по продвижению', services };
}
...
async function main() {
  const htmlFile = path.join(SRC_DIR, `${ADS_TEMPLATE_SLUG}.html`);
  ...
  try {
    ...
    // Get page data
    const pageResult = await client.query('SELECT * FROM pages WHERE slug = $1', [`/${ADS_TEMPLATE_SLUG}`]);
    ...
    await client.query(
      ...
      [
        ADS_TEMPLATE_SLUG,
        header?.title || 'Контекстная реклама',
        description?.text || '',
        priceCents,
        'RUB',
        'one_time',
    ...
    console.log(`Imported /${ADS_TEMPLATE_SLUG} as product template with ${Object.keys(contentJson).length} sections`);
  } catch (e) {
