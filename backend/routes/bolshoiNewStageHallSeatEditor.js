/**
 * Редактор координат/разметки мест — Большой театр, Новая сцена.
 */
...
const BUNDLE_PATH = path.join(
  REPO_ROOT,
  'backend/data/bolshoi-geodesy/hand/bolshoi-new-stage-seats.bundle.json',
);
const SVG_PUBLIC = '/hall-maps/bolshoi-new-stage.svg';
const ENRICHED_SVG_PUBLIC = 'bolshoi-new-stage-enriched.svg';
const DEFAULT_HALL_W = 1569;
const DEFAULT_HALL_H = 1104;
...
router.get('/status', async (_req, res) => {
  try {
    ...
    return res.json({
      ...
      editorUrl: '/tools/bolshoi-new-stage-editor.html',
    });
  } catch (e) {
...
router.get('/bundle', async (_req, res) => {
  try {
    const row = await loadStageMapRow();
    if (!row) {
      return res.status(404).json({ ok: false, error: 'stage map not seeded — npm run seed:bolshoi-new-stage-map' });
    }
    const layout = row.layout_json && typeof row.layout_json === 'object' ? row.layout_json : {};
