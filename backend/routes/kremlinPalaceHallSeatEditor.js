/**
 * Редактор координат/разметки мест — Государственный Кремлёвский дворец.
 */
...
const BUNDLE_PATH = path.join(
  REPO_ROOT,
  'backend/data/kremlin-palace/hand/kremlin-palace-seats.bundle.json',
);
const SVG_PUBLIC = '/hall-maps/kremlin-palace.svg';
const ENRICHED_SVG_PUBLIC = 'kremlin-palace-enriched.svg';
const DEFAULT_HALL_W = 3581;
const DEFAULT_HALL_H = 3052;
...
function hallDimensions(layout) {
  ...
  const tl = layout?.ticketland && typeof layout.ticketland === 'object' ? layout.ticketland : {};
  return {
    hallW: Number(pb.hallWidth) || Number(tl.hallWidth) || DEFAULT_HALL_W,
    hallH: Number(pb.hallHeight) || Number(tl.hallHeight) || DEFAULT_HALL_H,
  };
}
...
const svgEditor = createHallSvgEditorHandlers({
  ...
  backupExistingFile,
  editorMode: 'bolshoi-luzhniki-editor',
});
...
router.get('/status', async (_req, res) => {
  try {
    ...
    return res.json({
      ...
      editorUrl: '/tools/kremlin-palace-editor.html',
    });
  } catch (e) {
...
router.get('/bundle', async (_req, res) => {
  try {
    const row = await loadStageMapRow();
    if (!row) {
      return res.status(404).json({ ok: false, error: 'stage map not seeded — npm run seed:kremlin-palace-map' });
    }
    const layout = row.layout_json && typeof row.layout_json === 'object' ? row.layout_json : {};
