...
// Импортируем pool только если нужна БД (опционально)
let pool = null;
try {
  pool = (await import('../db.js')).default;
} catch (e) {
  // Игнорируем ошибку импорта БД - роут будет работать без БД
  console.warn('[Errors] Database not available, errors will be logged only to console');
}
...
router.post('/', async (req, res) => {
  try {
    ...
    // Всегда возвращаем успех, чтобы не ломать фронтенд
