...
2. **a101** (угловые): **cloudRowSeat** → **radialGrid+d124step** → pbiletLabeled (только доверенный prod, **не** `layout.seats` / fieldGrid pilot)  
3. strict / cloudMaster для остальных  
4. **Не** писать cloud/radial sellable обратно в `layout.seats` для a101 (иначе цикл «ложного pbiletLabeled»)  
3. **b154** → axisGrid  
4. fieldGrid / svgRow  
