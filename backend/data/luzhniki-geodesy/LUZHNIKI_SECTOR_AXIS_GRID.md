...
2. **pbiletLabeled** — exact `lookupLabeledSeat` в `layout.seats` / prod pilot bundle (≠ ближайшая серая из `coordinates`)  
3. cloudMaster (если `LUZHNIKI_CLOUD_MASTER=1`)  
4. **a101, b155, b156** → radialGrid+d124step (`seatSpacingScale` разводит места вдоль хорды)  
3. **b154** → axisGrid  
4. fieldGrid / svgRow  
