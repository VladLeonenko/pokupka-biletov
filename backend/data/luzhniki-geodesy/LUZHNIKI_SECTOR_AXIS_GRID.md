...
2. **a101**: **radialGrid+d124step** (§3, `seatSpacingScale`) — основной путь; **cloudRowSeat** только если radial не дал точку  
3. pbiletLabeled — только доверенный prod, **не** `layout.seats` / fieldGrid  
4. **Не** писать cloud/radial sellable обратно в `layout.seats` для a101  
3. **b154** → axisGrid  
4. fieldGrid / svgRow  
