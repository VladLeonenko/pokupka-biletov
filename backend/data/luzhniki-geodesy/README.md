...
**План: SVG viewBox, места без «скачка»** — [LUZHNIKI_SVG_VIEWBOX_COORDINATE_PLAN.md](./LUZHNIKI_SVG_VIEWBOX_COORDINATE_PLAN.md)

**✅ Пилот D 230 (эталон, 17.05.2026):** [worklog § Успешный пилот](./LUZHNIKI_STADIUM_MAP_WORKLOG.md#успешный-пилот-d-230--канон-для-всего-стадиона-17052026) — так же делаем остальные сектора.
...
- **Пилот / svgCircle (канон с 17.05.2026):** координаты sellable из **невидимых** `<circle place-name row place>` в `svg_markup` (слой `#luzhniki-pilot-seats`), источник координат — **`tickets.json` + интерполяция рядов** (`interpolatePbiletSeatGeodesy`). См. worklog «Успешный пилот D 230».
- **Не использовать** `layout.seats` / fieldGrid для офферов — сдвиг рядов (пример: API ряд 24 → на карте позиция ~ряда 28).
- Серая чаша — по-прежнему `allSeatCoordinates` (~77k), цветные точки — `sellableSeats` с API.
- Частичный `tickets.json` (~6k): для отсутствующих рядов — интерполяция между известными рядами сектора; для новых секторов — тот же пайплайн, что D 230.
