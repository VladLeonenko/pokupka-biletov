/**
 * Эвристики категории события — enrich, контекст репертуара, скрипты БД.
 * Держим в паре с frontend/src/utils/eventTitleHeuristics.ts (classifyEventTitle).
 *
 * @param {string} title
 * @param {{ subtitle?: string, genre?: string }} [ctx]
 * @returns {{ categoryLabel: string, descriptionBlurb: string, kind: 'sport' | 'football' | 'concert' | 'theater' | 'kids' | 'default' }}
 */
export function classifyEventTitle(title, ctx = {}) {
  function norm(s) {
    return (s || '').trim();
  }

  function buildCorpus(t, c) {
    const parts = [norm(t)];
    if (norm(c.subtitle)) parts.push(norm(c.subtitle));
    if (norm(c.genre)) parts.push(norm(c.genre));
    return parts.join('\n');
  }
  ...
  function looksTheaterishNotSport(t) {
    return /спектакль|читает\s|театр\.?\s*$|опера\.?\s*$|лекци|экскурс|чехов|гоголь|пьес|актёр|трагед|комедия\s*$/i.test(
      t,
    );
  }

  function likelyTeamVersusTitle(t) {
    const n = norm(t);
    if (!/[—–-]/.test(n)) return false;
    if (looksTheaterishNotSport(n)) return false;
    const chunks = n.split(/\s*[—–-]\s*/).map((x) => x.trim()).filter(Boolean);
  ...
  function genreHintsSport(genre) {
  ...
  function genreHintsTheater(genre) {
    const g = lo(norm(genre));
    if (!g) return false;
  ...
  function genreHintsConcert(genre) {
    const g = lo(norm(genre));
    if (!g) return false;
  ...
  const t = norm(title);
  const corpus = buildCorpus(title, ctx);
  const c = lo(corpus);
  ...
  const football =
    ...
    (/футбол|football|soccer|futbol/i.test(c) ||
      ...
      (genreHintsSport(ctx.genre) && /футбол|soccer|football|рпл/i.test(c + ' ' + lo(norm(ctx.genre)))) ||
      (likelyTeamVersusTitle(t) &&
  ...
  /** @type {'sport' | 'football' | 'concert' | 'theater' | 'kids' | 'default'} */
  let kind = 'default';
  ...
  return { categoryLabel, descriptionBlurb, kind };
}
