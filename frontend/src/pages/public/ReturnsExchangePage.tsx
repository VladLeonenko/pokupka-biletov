...
import proseStyles from './PrivacyPolicyPage.module.css';

function ReturnsStaticBody() {
  return (
    <div className={styles.prose}>
      <p>
        Мы выступаем как сервис продажи билетов: оформляем заказ и передаём данные организатору. Решение о возврате
        денежных средств и возможности обмена принимает организатор (и/или площадка) в соответствии с законодательством
        РФ и условиями конкретного мероприятия.
      </p>
      ...
      <ul className={styles.list}>
        ...
        <li>
          Если мероприятие отменено или перенесено, действуют положения о защите прав потребителей и правила продавца;
          условия возврата могут быть расширены организатором.
        </li>
      ...
      <p>
        Опишите номер заказа, мероприятие и причину запроса в форме на странице контактов. Мы передаём обращение в
        поддержку и фиксируем обращение в рабочее время. На срочные вопросы удобнее ответить по телефону — если номер
        указан в контактах на сайте.
      </p>

      <h2 className={styles.sectionTitle}>4. Отказ в переносе/возврате по инициативе зрителя</h2>
      <p>
        Если мероприятие состоится в заявленные сроки, односторонний отказ от билета регулируется правилами организатора и
        договором оферты. Отдельные категории билетов могут быть невозвратными — это должно быть доведено до покупателя
        до оплаты.
      </p>

      <div className={styles.note}>
        <span className={styles.noteStrong}>Не юридическая консультация.</span> Текст носит справочный характер. При споре
        с продавцом вы вправе обратиться к организатору, в площадку или в надзорные / судебные органы с учётом вашей
        ситуации.
      </div>
...
export function ReturnsExchangePage() {
  ...
  const cmsHtml = vitrine.returnsPolicyHtml?.trim() ?? '';
      ...
      <main className={styles.page}>
        ...
        <div className={styles.shell}>
          ...
          <article className={styles.card}>
            {cmsHtml ? (
              <div className={proseStyles.prose} dangerouslySetInnerHTML={{ __html: cmsHtml }} />
            ) : (
              <ReturnsStaticBody />
            )}
          </article>
