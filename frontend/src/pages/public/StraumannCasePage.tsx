...
/**
 * Страница кейса "Straumann" - полностью на React компонентах
 * Сохраняет структуру, стили и наполнение из БД
 */
export function StraumannCasePage() {
  useCursor();
  
  const { data: page, isLoading } = useQuery({
    queryKey: ['public-page', '/straumann-case'],
    queryFn: () => getPublicPage('/straumann-case'),
    staleTime: 30000,
  });

  useEffect(() => {
    document.body.setAttribute('data-page', '/straumann-case');
  ...
  const currentUrl = typeof window !== 'undefined' ? window.location.href : 'https://primecoder.ru/straumann-case';
