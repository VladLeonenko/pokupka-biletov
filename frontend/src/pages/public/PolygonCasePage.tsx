...
/**
 * Страница кейса "Полигон" - полностью на React компонентах
 * Сохраняет структуру, стили и наполнение из БД
 */
export function PolygonCasePage() {
  useCursor();
  
  const { data: page, isLoading } = useQuery({
    queryKey: ['public-page', '/polygon'],
    queryFn: () => getPublicPage('/polygon'),
    staleTime: 30000,
  });

  useEffect(() => {
    document.body.setAttribute('data-page', '/polygon');
  ...
  const currentUrl = typeof window !== 'undefined' ? window.location.href : 'https://primecoder.ru/polygon';
