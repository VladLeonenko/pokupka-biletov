...
import { sendAIMessageAuth, AIMessage } from '@/services/aiChatApi';
...
export function AIChatPage() {
  ...
  // Воспроизведение ответа AI голосом
  const speakText = (text: string) => {
    if (!synthRef.current || !isVoiceEnabled) return;

    synthRef.current.cancel(); // Отменяем предыдущее воспроизведение
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await sendAIMessageAuth(userMessage.content, messages);
      ...
      // Автоматически воспроизводим ответ AI голосом
      if (isVoiceEnabled) {
        ...
        >
          ...
          <Tooltip title={isListening ? 'Остановить запись' : 'Голосовой ввод'}>
            <IconButton
              ...
              sx={{
                bgcolor: isListening ? 'error.light' : 'primary.main',
                color: 'white',
                '&:hover': {
