// Оптимизированный review.js для главной страницы
(function() {
  'use strict';

  function initReviewTabs() {
    const head = document.querySelector('.review-tabs');
    ...
    if (!head.querySelector('.tabs__caption_active')) {
      const firstCaption = head.querySelector('.review__tabs__caption');
      if (firstCaption) {
    ...
    head.addEventListener('click', function(e) {
      const caption = e.target.closest('.review__tabs__caption');
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initReviewTabs);
  } else {
    initReviewTabs();
  }
})();
